...
bool quennStep(int tempFigure, int tempFinalPoint, Figure** map, int size, int tempFigure2, int tempFinalPoint2, bool botMode, bool matCheck)
{
	...
	{
		bool res = (rStep(tempFigure, tempFinalPoint, map, size, tempFigure2, tempFinalPoint2, botMode, matCheck)) ? true : false;
		if(res)
			map[size - (tempFinalPoint2)][tempFinalPoint - 1].setValue('q', map[size - (tempFinalPoint2)][tempFinalPoint - 1].getColor());
		return (res) ? true : false;
	}
	else
	{
		bool res = (eStep(tempFigure, tempFinalPoint, map, size, tempFigure2, tempFinalPoint2, botMode, matCheck)) ? true : false;
		if(res)
			map[size - (tempFinalPoint2)][tempFinalPoint - 1].setValue('q', map[size - (tempFinalPoint2)][tempFinalPoint - 1].getColor());
		return (res) ? true : false;
	}
